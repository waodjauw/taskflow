<template>
  <aside class="sidebar">
    ...
    <div v-for="item in navItems" :key="item.nav"
      ...
      @click="store.setActiveNav(item.nav)">
    ...
    <div v-for="p in priorities" :key="p.nav"
      ...
      @click="store.setActiveNav(p.nav)">
    ...
    <div v-for="cat in store.categories" :key="cat.id"
      ...
      @click="store.setActiveNav('cat-' + cat.id)">
...
const navItems = [
  { nav: 'all', label: '全部任务', icon: Layers, badgeKey: 'all', badgeClass: '' },
  { nav: 'today', label: '今日', icon: Calendar, badgeKey: 'today', badgeClass: '' },
  { nav: 'week', label: '本周', icon: CalendarRange, badgeKey: 'week', badgeClass: '' },
  { nav: 'overdue', label: '已逾期', icon: AlertTriangle, badgeKey: 'overdue', badgeClass: 'danger' },
  { nav: 'done', label: '已完成', icon: CheckCircle2, badgeKey: 'done', badgeClass: '' },
]

const priorities = [
  { nav: 'p-critical', label: '紧急', color: '#ef4444', badgeKey: 'critical' },
  { nav: 'p-high', label: '高', color: '#f97316', badgeKey: 'high' },
  { nav: 'p-medium', label: '中', color: '#f59e0b', badgeKey: 'medium' },
  { nav: 'p-low', label: '低', color: '#22c55e', badgeKey: 'low' },
]
</script>
